...
/** Publishes additive regular OrderConfirmed V2 facts after their local outbox transaction commits. */
public final class KafkaOrderConfirmedV2Publisher implements PublishOrderEventPort {
    private final KafkaTemplate<String, OrderConfirmedV2> kafka;
    private final OrderConfirmedV2AvroMapper mapper;
    private final OrderKafkaProperties properties;

    public KafkaOrderConfirmedV2Publisher(KafkaTemplate<String, OrderConfirmedV2> kafka,
            OrderConfirmedV2AvroMapper mapper, OrderKafkaProperties properties) {
    ...
    public void publish(OrderOutboxEvent event) {
        var record = new ProducerRecord<String, OrderConfirmedV2>(properties.orderEventsTopic(), event.eventKey(),
                mapper.map(event));
        ...
        try {
            ...
            throw new IllegalStateException("OrderConfirmedV2 publication interrupted", exception);
        } catch (Exception exception) {
            throw new IllegalStateException("OrderConfirmedV2 publication failed", exception);
        }
    }
