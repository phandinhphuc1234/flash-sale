    ...
    <main className="mx-auto min-h-[70vh] max-w-3xl px-6 py-12">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          {order && <p className="mt-1 text-sm text-gray-500">{order.purchaseSource === "CART" ? "Cart checkout" : order.purchaseSource === "BUY_NOW" ? "Buy now" : "Flash Sale"}</p>}
        </div>
        {order && <span className="rounded-full bg-orange-50 px-3 py-1 text-sm text-orange-700">{order.status}</span>}
      </div>
      ...
      {order && <div className="mt-8 rounded-2xl border bg-white p-5 shadow-sm">
        <div className="space-y-3">
          {order.items.map((item) => <div key={item.variantId} className="flex justify-between gap-4"><span>{item.quantity} × {item.variantId}</span><span>{formatVnd(item.lineAmount)}</span></div>)}
        </div>
        ...
        {payment ? <div className="mt-5 rounded-xl bg-gray-50 p-4 text-sm"><div className="flex justify-between"><span>Payment</span><span className="font-medium">{payment.status}</span></div>{payment.failureReason && <p className="mt-2 text-red-700">{payment.failureReason}</p>}</div> : ["PENDING_PAYMENT", "CONFIRMED"].includes(order.status) && <p className="mt-5 rounded-xl bg-orange-50 p-4 text-sm text-orange-800">Payment is being prepared asynchronously. Refresh in a moment.</p>}
        {payment && !["SUCCEEDED", "EXPIRED"].includes(payment.status) && <button disabled={pending} onClick={checkout} className="mt-6 w-full rounded-xl bg-orange-600 py-3 font-medium text-white disabled:opacity-60">{pending ? "Opening secure checkout…" : "Pay securely with Stripe"}</button>}
