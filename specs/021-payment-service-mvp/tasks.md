...
- [ ] T007 Add idempotent local command/event/DLT topic provisioning plus controlled BACKWARD_TRANSITIVE Payment subject registration/check-only workflows in `infra/docker/kafka/init-payment-topics.sh` and `infra/docker/schema-registry/register-payment-schemas.ps1`
- [ ] T008 Create the implementation evidence ledger, run contract generation/schema compatibility plus the empty Payment application context, and record approved artifact/ADR/schema/topic baselines, commands, generated records, test counts, and exit status in `specs/021-payment-service-mvp/validation.md`
...
- [ ] T054 [P] [US3] Add exact raw-body signature, timestamp tolerance, wrong secret/live-mode/expected-API-version, malformed/unsupported event, and redaction tests in `services/payment-service/src/test/java/com/philia/flashsale/payment/payment/adapter/in/webhook/StripeWebhookVerifierTests.java` (FR-019, NFR-SEC-002)
