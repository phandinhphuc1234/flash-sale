...
# Provision only the approved Feature 017 topic. This script is intentionally root-owned so
# service modules do not own shared broker lifecycle or topic administration.
...
TOPIC="${CAMPAIGN_KAFKA_TOPIC:-campaign.lifecycle.v1}"
PARTITIONS="${CAMPAIGN_KAFKA_PARTITIONS:-3}"
REPLICATION_FACTOR="${CAMPAIGN_KAFKA_REPLICATION_FACTOR:-1}"

# Git Bash on Windows exposes paths as /c/... while the Docker Desktop CLI expects a native
# Windows path for --env-file and -f. Linux/macOS shells keep their absolute paths unchanged.
compose_path() {
...
# Relative paths avoid an additional Windows path conversion when this script is launched from
# Git Bash. The default environment file lives next to compose.yml, so this is the normal branch.
