...
public final class OrderCancelledAvroMapper {
    ...
    public OrderCancelledAvroMapper(ObjectMapper objectMapper) { this.objectMapper = objectMapper; }
    public OrderCancelledV1 map(OrderOutboxEvent event) {
        if (event == null || !"OrderCancelled".equals(event.eventType()) || event.eventVersion() != 1 || !"ORDER".equals(event.aggregateType())) throw invalid("outbox envelope is not OrderCancelled.v1");
        JsonNode root; try { root = objectMapper.readTree(event.payload()); } catch (IOException exception) { throw invalid("OrderCancelled payload is not valid JSON"); }
        UUID orderId = uuid(root, "orderId"); if (!orderId.equals(event.aggregateId()) || !event.eventKey().equals(orderId.toString())) throw invalid("OrderCancelled identity/key mismatch");
        return new OrderCancelledV1(event.eventId(), event.eventType(), event.eventVersion(), "order-service", event.aggregateType(), event.aggregateId(), event.aggregateVersion(), event.correlationId(), event.causationId(), event.occurredAt(), new OrderCancelledDataV1(orderId, text(root, "orderNumber"), uuid(root, "purchaseRequestId"), uuid(root, "reservationId"), text(root, "reason"), instant(root, "cancelledAt")));
    }
