...
export default function ReservationPage() {
  ...
  const reservationCopy = getStatusCopy("reservation", reservation?.status);
  return <><Navbar /><main className="mx-auto min-h-[70vh] max-w-xl px-6 py-20 text-center"><div className="rounded-lg bg-gray-50 p-8"><p className="text-sm text-orange-600">Reservation status</p><h1 className="mt-2 text-2xl font-medium">{reservation ? reservationCopy.label : "Processing reservation"}</h1><p className="mt-2 text-sm text-gray-500">{reservation ? reservationCopy.guidance : "We are checking the reservation."}</p><p className="mt-4 text-gray-600">{message}</p>{reservation?.expiresAt && <p className="mt-3 text-sm text-gray-500">Reserved until {new Date(reservation.expiresAt).toLocaleString()}</p>}<ApiNotice error={error} className="mt-5 text-left" /><button onClick={() => router.push("/my-orders")} className="mt-6 rounded bg-orange-600 px-5 py-2.5 text-white">View my orders</button></div></main></>;
}
